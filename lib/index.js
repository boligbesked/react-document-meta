...
function removeMetaDuplicated(metaItem) {
  const nameField = metaItem.hasOwnProperty('name') ? 'name' : 'property';
  const item = document.querySelector( metaItem.tagName + '['+nameField+'="'+metaItem[nameField]+'"]' );
...
function insertDocumentMeta ( props ) {
  removeDocumentMeta();
  if (props.hasOwnProperty('force') && props.force) forEach( getTags( props ), removeMetaDuplicated);
